...
```bash
# 模式1：数据集评测 (指定11-30号歌曲，含.pv准确率对比+折线图)
python demo.py  # 选 1，输入歌曲编号如 11
...
### demo.py — 整合入口
交互式选择两种模式：
- **模式1**：选择编号（11-30），加载 `task2/` 中的 .wav 和 .pv，输出严格/八度容错准确率 + matplotlib 折线图
- **模式2**：自动调用录音 → 子序列 DTW 检索 `midi_cache/` 中 20 首曲库 → 打印 Top-10 排名 → 可选 pygame/系统播放器播放第一候选 MIDI

## 目录结构

```
demo_samples/
  task2/00011-00030.{wav,pv}  # 20对 MIR-QBSH 测试样本+人工标注(缺00028.pv/wav)
  进阶1/live_test.wav          # 模式2录音输出路径
  midiFile/00009-00030.mid    # 22首标准乐谱
midi_cache/00011-00030.npy     # 20首预计算 pitch contour (缺00009-00010)
```

## 数据集

MIR-QBSH 为核心数据集（MIREX 官方 QBSH 评测集），不在仓库内，应放在 `../MIR-QBSH/`。`task2/` 中的 20 对 .wav/.pv 即来自该数据集。
